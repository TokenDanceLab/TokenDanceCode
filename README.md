...
`config set` 只写入 `provider`、`model`、`permissionMode` 到 `.tokendance/config.json` 或 `~/.tokendance/config.json`，并会拒绝 `apiKey`、`token`、`secret` 等非白名单字段。Provider key 继续放在受控环境变量或全局 `~/.tokendance/.env`，不要写入 JSON 配置。
