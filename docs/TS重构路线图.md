...
- 当前可验证命令：`pnpm verify`
- 最近验证结果：typecheck 通过，Vitest 17 个测试文件 68 个测试通过。
...
- [x] 顶层 `resume [session-id]`、交互式 `/resume`、`/compact`。
- [x] 滚动式事件 renderer 最小闭环：assistant 文本、tool started、permission decision、tool completed、tool failed reason。
- [ ] 增强 renderer：大输出摘要、未来真实 provider token delta。
