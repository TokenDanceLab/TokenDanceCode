...
- 最近验证结果：`pnpm release:next:check` 通过，覆盖 typecheck、Vitest 26 个测试文件 197 个测试、core/sdk/cli dry-run pack 和增强 tarball install smoke。
...
### P5：稳定化与 AgentHub 消费打磨

- [x] CLI command architecture：抽出 `packages/cli/src/commands.ts` 负责顶层 argv 解析和分派，`packages/cli/src/main.ts` 保持 IO-aware handler wiring，避免继续把所有行为塞进薄入口。
- [x] AgentHub consuming fixture：把 SDK manifest、doctor、resume-or-start、event sink、approval bridge 和 TokenDanceID login facade 串成更接近 AgentHub 的可复制样例。
- [x] 滚动式 TUI polish：继续改善颜色、错误块、权限/风险/usage 输出，同时保持无颜色输出可测试。
