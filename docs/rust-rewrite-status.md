...
| Slice | Target |
|---|---|
| Real provider HTTP | gated OpenAI-compatible smoke without printing credentials |
| File/shell tools | read/write/glob, PowerShell classifier, path subjects |
| Runtime tool loop | provider tool calls, permission events, tool results, model-call limit |
| SDK runner parity | context preview, approval bridge wired into runtime, richer AgentHub terminal frames |
| Package smoke | local tarball install of wrapper plus current-platform binary |

## Release Boundary

This branch is not ready for npm or GitHub Release publication. The next release candidate requires real provider transport or gated smoke, file/shell tool execution, runtime tool-loop parity, npm wrapper tarball smoke, tarball privacy scans, and full CLI/SDK contract parity tests.
