...
- 当前可验证命令：`pnpm verify`
- 最近验证结果：typecheck 通过，Vitest 3 个测试文件 7 个测试通过。
...
- [ ] File tools：read/write/edit/glob，全部走 permission engine。
- [ ] PowerShell shell tool：风险分类、工作区边界、输出截断。
- [ ] Patch tool：结构化 apply patch，失败可诊断。
