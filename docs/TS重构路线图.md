...
- 当前可验证命令：`pnpm verify`
- 最近验证结果：typecheck 通过，Vitest 20 个测试文件 93 个测试通过。
...
- [x] Worktree manager：受控 `.worktrees` list/create/remove，dirty 删除需显式 `--discard`。
...
- [x] 顶层 `config`、`resume [session-id]`、`memory [add|delete] [project|global] [value]`、`tasks [create|doing|done] [value]`、`todo [add|doing|done] [value]`、`worktree [list|create|remove] [name] [--discard]`、`diff [path ...]`、`review`、`quality <command>`、`transcript [session-id]`、`transcript [session-id] search <query>`、`compact [session-id]`，交互式 `/config`、`/resume`、`/memory`、`/tasks`、`/todo`、`/worktree`、`/diff`、`/review`、`/quality`、`/transcript`、`/transcript search <query>`、`/compact`。
