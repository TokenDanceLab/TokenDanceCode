...
- 当前可验证命令：`pnpm verify`
- 最近验证结果：typecheck 通过，Vitest 23 个测试文件 114 个测试通过。
...
- [x] Doctor 诊断：Node/cwd/platform、API key present/missing、Git/PowerShell 可用性、config 路径/source、`.tokendance` 状态目录可写性；不输出 secret 值。
